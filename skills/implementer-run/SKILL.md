...
Before inspecting changes or performing cleanup, exclude `$BUG_LABEL` from processing recovery.
`bugfixer-run` exclusively owns bug recovery; do not reset, relabel, comment on,
clean up a checkout for, or otherwise mutate a processing bug from this role.
